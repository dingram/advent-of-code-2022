...
def main(argv0: str, args: Sequence[str]) -> None:
  ...
  with open(args[0], 'rt') as input_file:
    ...
    currently_free = filesystem_size - directory_tree['/']
    amount_to_delete = wanted_free - currently_free
    print(f'{currently_free=}')
    print(f'{amount_to_delete=}')

    # Create a dict of just the directories.
    directories = {
        path: size for path, size in directory_tree.items()
        if path.endswith('/')}
    # Loop through directories in order of increasing size.
    for path, size in sorted(directories.items(), key=lambda i: i[1]):
      # Find the first one that's big enough, and output it.
